...
mod tests {
    ...
    fn test_new_basic() {
        ...
        assert_eq!(amount.symbol, Some("ETH".to_string()));
    }

    #[test]
    fn test_new_without_symbol() {
        let amount = TokenAmount::new(1000000, 6, None);
    ...
    #[test]
    fn test_new_zero() {
        let amount = TokenAmount::new(0, 18, Some("ETH".to_string()));
    ...
    fn test_new_large_amount() {
        let amount = TokenAmount::new(u128::MAX, 18, None);
    ...
    fn test_from_human_whole_number() {
        let amount = TokenAmount::from_human("100", 18, Some("ETH".to_string())).unwrap();
    ...
    fn test_from_human_usdc_whole() {
        let amount = TokenAmount::from_human("100", 6, Some("USDC".to_string())).unwrap();
        assert_eq!(amount.raw, 100000000);
        assert_eq!(amount.decimals, 6);
        assert_eq!(amount.symbol, Some("USDC".to_string()));
    }

    #[test]
    fn test_from_human_usdc_decimal() {
        let amount = TokenAmount::from_human("1.5", 6, Some("USDC".to_string())).unwrap();
    ...
    fn test_from_human_zero() {
        let amount = TokenAmount::from_human("0", 18, None).unwrap();
        assert_eq!(amount.raw, 0);
        assert_eq!(amount.decimals, 18);
    }
    ...
    fn test_from_human_zero_decimal() {
        let amount = TokenAmount::from_human("0.0", 18, None).unwrap();
        assert_eq!(amount.raw, 0);
    }
    ...
    fn test_from_human_precise_decimal() {
        ...
        let amount = TokenAmount::from_human("1.234567890123456789", 18, None).unwrap();
    ...
    fn test_from_human_precise_decimal_6() {
        ...
        let amount = TokenAmount::from_human("1.123456", 6, None).unwrap();
        assert_eq!(amount.raw, 1123456);
    }

    #[test]
    fn test_from_human_very_large_integer() {
        let amount = TokenAmount::from_human("1000000000", 18, None).unwrap();
        assert_eq!(amount.raw, 1000000000000000000000000000);
    }

    #[test]
    fn test_from_human_leading_zeros_integer() {
        let amount = TokenAmount::from_human("0001.5", 18, None).unwrap();
        assert_eq!(amount.raw, 1500000000000000000);
    }

    #[test]
    fn test_from_human_trailing_zeros_fractional() {
        let amount = TokenAmount::from_human("1.500", 18, None).unwrap();
        assert_eq!(amount.raw, 1500000000000000000);
    }
    ...
    fn test_from_human_invalid_format_multiple_dots() {
        let result = TokenAmount::from_human("1.2.3", 18, None);
    ...
    fn test_from_human_fractional_exceeds_decimals() {
        let result = TokenAmount::from_human("1.1234567890123456789", 18, None);
    ...
    fn test_from_human_fractional_exceeds_decimals_6() {
        let result = TokenAmount::from_human("1.1234567", 6, None);
        assert!(result.is_err());
        assert!(result.unwrap_err().contains("Fractional part has"));
    }
    ...
    fn test_from_human_invalid_integer() {
        let result = TokenAmount::from_human("abc", 18, None);
        assert!(result.is_err());
    ...
    fn test_from_human_invalid_fractional() {
        let result = TokenAmount::from_human("1.abc", 18, None);
        assert!(result.is_err());
    ...
    fn test_from_human_empty_string() {
        let result = TokenAmount::from_human("", 18, None);
        assert!(result.is_err());
    }
    ...
    fn test_from_human_only_dot_start() {
        let result = TokenAmount::from_human(".5", 18, None);
    ...
    fn test_from_human_overflow() {
        ...
        let _result = TokenAmount::from_human("340282366920938463463374607431768211456", 0, None);
        ...
        let _result2 = TokenAmount::from_human("999999999999999999999999999999999999999", 18, None);
    ...
    fn test_human_whole_number() {
        let amount = TokenAmount::new(100000000000000000000, 18, None);
        assert_eq!(amount.human(), "100");
    }

    #[test]
    fn test_human_decimal() {
        let amount = TokenAmount::new(1500000000000000000, 18, None);
        assert_eq!(amount.human(), "1.5");
    }

    #[test]
    fn test_human_small_decimal() {
        let amount = TokenAmount::new(1000000000000000, 18, None);
        assert_eq!(amount.human(), "0.001");
    }

    #[test]
    fn test_human_zero() {
        let amount = TokenAmount::new(0, 18, None);
        assert_eq!(amount.human(), "0");
    }

    #[test]
    fn test_human_precise_decimal() {
        let amount = TokenAmount::new(1234567890123456789, 18, None);
        assert_eq!(amount.human(), "1.234567890123456789");
    }

    #[test]
    fn test_human_usdc_whole() {
        let amount = TokenAmount::new(100000000, 6, None);
    ...
    fn test_human_usdc_decimal() {
        let amount = TokenAmount::new(1500000, 6, None);
        assert_eq!(amount.human(), "1.5");
    }
    ...
    fn test_human_single_digit_fractional() {
        let amount = TokenAmount::new(500000000000000000, 18, None);
        assert_eq!(amount.human(), "0.5");
    }

    #[test]
    fn test_human_very_small_amount() {
        let amount = TokenAmount::new(1, 18, None);
        assert_eq!(amount.human(), "0.000000000000000001");
    }

    #[test]
    fn test_human_large_integer() {
        let amount = TokenAmount::new(1000000000000000000000000000, 18, None);
        assert_eq!(amount.human(), "1000000000");
    }

    #[test]
    fn test_human_6_decimals() {
        let amount = TokenAmount::new(123456, 6, None);
    ...
    fn test_human_8_decimals() {
        ...
        let amount = TokenAmount::new(12345678, 8, None);
    ...
    fn test_human_0_decimals() {
        ...
        let amount = TokenAmount::new(100, 0, None);
        assert_eq!(amount.human(), "100");
    }

    #[test]
    fn test_human_1_decimal() {
        let amount = TokenAmount::new(15, 1, None);
        assert_eq!(amount.human(), "1.5");
    }
    ...
    fn test_round_trip_whole_number() {
        let original = "100";
        let amount = TokenAmount::from_human(original, 18, None).unwrap();
    ...
    fn test_round_trip_usdc() {
        let original = "1.5";
        let amount = TokenAmount::from_human(original, 6, None).unwrap();
        assert_eq!(amount.human(), original);
    }
    ...
    fn test_different_decimal_precisions() {
        ...
        for decimals in [0, 1, 6, 8, 18] {
            let amount = TokenAmount::new(100, decimals, None);
    ...
    fn test_very_small_fractional() {
        ...
        let amount = TokenAmount::from_human("0.000000000000000001", 18, None).unwrap();
    ...
    fn test_integer_part_zero_with_fractional() {
        let amount = TokenAmount::from_human("0.5", 18, None).unwrap();
    ...
    fn test_add_same_decimals() {
        ...
        assert_eq!(sum.symbol, Some("ETH".to_string()));
    }

    #[test]
    #[should_panic(expected = "DecimalMismatch")]
    fn test_add_different_decimals() {
        let a = TokenAmount::new(1000000000000000000, 18, None);
        let b = TokenAmount::new(1000000, 6, None);
    ...
    fn test_add_zero() {
        let a = TokenAmount::new(1000000000000000000, 18, Some("ETH".to_string()));
        let b = TokenAmount::new(0, 18, Some("ETH".to_string()));
        let sum = a + b;
    ...
    fn test_add_symbol_handling() {
        // When first has symbol, use it
        let a = TokenAmount::new(1000000000000000000, 18, Some("ETH".to_string()));
        let b = TokenAmount::new(500000000000000000, 18, None);
        let sum = a + b;
        assert_eq!(sum.symbol, Some("ETH".to_string()));

        // When first doesn't have symbol, use second's
        let a2 = TokenAmount::new(1000000000000000000, 18, None);
        let b2 = TokenAmount::new(500000000000000000, 18, Some("ETH".to_string()));
        let sum2 = a2 + b2;
        assert_eq!(sum2.symbol, Some("ETH".to_string()));
    }
    ...
    fn test_try_add_decimal_mismatch() {
        ...
        match result.unwrap_err() {
            ...
            _ => panic!("Expected DecimalMismatch error"),
        }
    }
    ...
    fn test_mul_u128() {
        ...
        assert_eq!(doubled.symbol, Some("ETH".to_string()));
    }

    #[test]
    fn test_mul_u64() {
        let amount = TokenAmount::new(1000000, 6, Some("USDC".to_string()));
    ...
    #[test]
    fn test_mul_u32() {
        let amount = TokenAmount::new(1000000000000000000, 18, None);
    ...
    fn test_display_whole_number_with_symbol() {
        let amount = TokenAmount::new(100000000000000000000, 18, Some("ETH".to_string()));
        let display = format!("{}", amount);
    ...
    fn test_display_precise_with_symbol() {
        let amount = TokenAmount::new(1234567890123456789, 18, Some("ETH".to_string()));
        let display = format!("{}", amount);
    ...
    fn test_display_usdc() {
        let amount = TokenAmount::new(1500000, 6, Some("USDC".to_string()));
        let display = format!("{}", amount);
